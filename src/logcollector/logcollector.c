...
void LogCollectorStart()
{
    ...
    for (i = 0;; i++) {
        ...
        else if (strcmp(current->logformat, "eventlog") == 0) {
#ifdef WIN32
        ...
        else if (strcmp(current->logformat, "eventchannel") == 0) {
#ifdef WIN32
            ...
            for (j=0; globs[j].gpath; j++) {
                ...
                while (g.gl_pathv[glob_offset] != NULL) {
                    ...
                    if (!found) {
                        ...
                        globs[j].gfiles[i].ffile = globs[j].gfiles->ffile;
                        globs[j].gfiles[i].logformat = globs[j].gfiles->logformat;
                        globs[j].gfiles[i].djb_program_name = globs[j].gfiles->djb_program_name;
                        globs[j].gfiles[i].command = globs[j].gfiles->command;
                        globs[j].gfiles[i].alias = globs[j].gfiles->alias;
                        globs[j].gfiles[i].future = globs[j].gfiles->future;
                        globs[j].gfiles[i].query = globs[j].gfiles->query;
                        globs[j].gfiles[i].labels = globs[j].gfiles->labels;
                        globs[j].gfiles[i].read = globs[j].gfiles->read;
